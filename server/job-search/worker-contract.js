...
function criticMessages(packet) {
  return [
    ...
    {
      role: "user",
      content: `Candidate profile:\n${packet.candidate.baseline}\n\nCore expertise:\n${JSON.stringify(packet.candidate.coreExpertise, null, 2)}\n\nEvaluation policy:\n${JSON.stringify(packet.candidate.evaluationPolicy, null, 2)}\n\nFour must-have gates:\n${JSON.stringify(packet.candidate.hardRequirements, null, 2)}\n\nJob and primary evaluation:\n${JSON.stringify({ job: packet.job, primary: packet.job.deepEvaluation, evidence: packet.evidence }, null, 2)}\n\nAudit the evaluation from scratch. First verify this is one specific vacancy, not a careers landing page, multi-role result page, or article. Look equally for false rejection and false optimism. Challenge responsibility-fit reasoning, unsupported gate statuses, score/reason contradictions, annual-base versus total-compensation mistakes, unrelated-company or unrelated-role research, hidden coding-interview risk, and work-authorization evidence. A public-sector or different-industry role can still be a strong expertise match. Multiple distinct mandatory specialist requirements absent from the résumé can reduce fit to 2 and block. A strategy-sounding title cannot override hands-on engineering duties. Legal counsel and federal legislative policy are separate primary functions; generic leadership does not satisfy fit. Lack of AI, financial-services overlap, remote work, or employer prestige cannot reduce core fit. Missing evidence cannot become score 0, met, or blocked. F-1/OPT/CPT acceptance alone cannot establish future sponsorship. Engineering and coding-bound scientist IC roles are near-certain coding risks absent contrary role-specific evidence; product/decision science and data-science management are elevated but not automatically blocked; analytics/strategy leadership is lower risk but unverified. Recalculate hourly pay with exactly 2,080 hours. A confirmed maximum below $170,000 and explicit no-sponsorship evidence are blockers; unknown gates cannot cancel them. Treat server-finalized deterministic gates as authoritative unless their cited source demonstrably contradicts them. A pass requires at least one supported blocker; unknown gates without a blocker require maybe. If any supported blocker remains, recommendedVerdict must be pass. apply means pursue, maybe means manual review, and pass means reject. Set agrees=true exactly when recommendedVerdict equals the primary final verdict. In objections, identify every material anomaly and state the corrected gate or dimension.`,
    },
  ];
}

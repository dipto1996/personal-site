...
The Windows worker performs deep evaluation in two separate calls: evidence extraction (maximum 1,800 output tokens) and evaluation (maximum 2,400). Triage is capped at 900, critic at 900, and outreach at 500. Every selected candidate, including a triage clear mismatch, receives a deep evaluation and then an independent critic. The worker runs one job at a time with an 8,192-token context and a 12,000-character job-description cap. On the measured Windows host it uses the low-heat profile: two CPU threads and eight GPU layers.

...
$env:JOBSEARCH_LOCAL_GPU_LAYERS = if ($null -ne $config.gpuLayers) { [string][Math]::Min(20, [int]$config.gpuLayers) } else { '20' }
$env:JOBSEARCH_LOCAL_CPU_THREADS = if ($null -ne $config.cpuThreads) { [string]$config.cpuThreads } else { '4' }
